...
if ( [ "${MULTI_REGION}" = "1" ] )
then
	/bin/echo "*/1 * * * * export HOME="${HOME}" && ${HOME}/services/dbaas/AdjustDBaaSFirewall.sh" >> /var/spool/cron/crontabs/root
fi
...
/bin/echo "@reboot export HOME="${HOME}" && ${HOME}/services/datastore/config/ActivateConfigDatastore.sh" >> /var/spool/cron/crontabs/root

if ( [ "`${HOME}/utilities/config/ExtractBuildStyleValues.sh "DATASTORECONFIGSTYLE" | /usr/bin/awk -F':' '{print $NF}'`" = "lightweight" ] )
then
	/bin/echo "@reboot export HOME="${HOME}" && ${HOME}/services/datastore/config/ActivateConfigDatastoreLightweight.sh" >> /var/spool/cron/crontabs/root
