...
if ( [ "${3}" = "" ] )
then
	private_server_ip="`${HOME}/services/server/GetServerPrivateIPAddressByIP.sh ${server_ip} ${cloudhost}`"
else 
	private_server_ip="${3}"
fi
...
then
	...
	if ( [ "${server_ip}" != "" ] )
	then
		server_to_delete="`${HOME}/services/server/GetServerName.sh ${server_ip} 'digitalocean'`"
...
then
	...
	then
		server_to_delete=""
		server_to_delete="`${HOME}/services/server/GetServerName.sh ${server_ip} 'linode'`"
