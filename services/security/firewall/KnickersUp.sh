...
then
	...
	if ( [ "${firewall}" = "ufw" ] )
	then
		/bin/echo ${SERVER_USER_PASSWORD} | /usr/bin/sudo -S /usr/sbin/ufw default deny incoming
		/bin/sleep 10
		/bin/echo ${SERVER_USER_PASSWORD} | /usr/bin/sudo -S /usr/sbin/ufw default allow outgoing
