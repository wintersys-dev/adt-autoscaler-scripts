	...
	then
       if ( [ "`/bin/grep "^CLOUDCLITOOL:*" ${HOME}/runtime/software.dat | /bin/grep CLOUDCLITOOL:linode-cli:snap`" != "" ] )
        then
				...
				${HOME}/utilities/processing/RunServiceCommand.sh snapd restart
        ...
        elif ( [ "`/bin/grep "^CLOUDCLITOOL:*" ${HOME}/runtime/software.dat | /bin/grep CLOUDCLITOOL:linode-cli:pip`" != "" ] )
        then
...
then
	${HOME}/services/email/SendEmail.sh "INSTALLATION ERROR LINODE-CLI" "I believe that linode cli hasn't installed correctly, please investigate" "ERROR"
else
