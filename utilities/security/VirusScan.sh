...
if ( [ "$?" != "0" ] )
then
  ${HOME}/services/email/SendEmail.sh "TROUBLE UPDATING VIRUS DATABASE" "Failed to update the virus database successfully" "ERROR"
fi
...
then
        ${HOME}/services/email/SendEmail.sh "TROUBLE PERFORMING PRODUCING VIRUS SCAN REPORT" "Failed to perform virus scan correctly" "ERROR"
else
  ...
  ${HOME}/services/email/SendEmail.sh "VIRUS SCAN REPORT FOR ${machine}" "${message}" "MANDATORY"
fi
