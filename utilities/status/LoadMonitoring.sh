...
then
	...
	then
		${HOME}/services/email/SendEmail.sh "HIGH PERCENTAGE LOAD" "More than a quarter of samples had more than 90% load over the past hour on machine with IP address `${HOME}/utilities/processing/GetPublicIP.sh`" "ERROR"
	fi
