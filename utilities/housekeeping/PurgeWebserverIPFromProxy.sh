...
proxy_ip_addresses="`${HOME}/services/server/GetServerPrivateIPAddresses.sh "rp-" "${CLOUDHOST}"`"

for ip_address in ${proxy_ip_addresses}
do
	/usr/bin/ssh -q -p ${SSH_PORT} -i ${BUILD_KEY} ${OPTIONS} ${SERVER_USER}@${ip_address} "${SUDO} /home/${SERVER_USER}/services/webserver/configuration/reverseproxy/RemoveOldIPFromReverseProxyIPList.sh ${webserver_ip}"
done

${HOME}/services/datastore/config/wrapper/DeleteFromDatastore.sh "config"  "webserverips/${webserver_ip}"
