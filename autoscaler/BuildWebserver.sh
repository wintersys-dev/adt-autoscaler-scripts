...
while ( [ "`${HOME}/services/server/IsInstanceRunning.sh ${webserver_name} ${CLOUDHOST}`" != "running" ] )
do
	/bin/sleep 5
done
...
do
	/bin/sleep 5
	/bin/echo "${0} `/bin/date`: Attempting to get ip address of webserver ${webserver_name} " 
	ip="`${HOME}/services/server/GetServerIPAddresses.sh ${webserver_name} ${CLOUDHOST}`"
	private_ip="`${HOME}/services/server/GetServerPrivateIPAddresses.sh ${webserver_name} ${CLOUDHOST}`"
	count="`/usr/bin/expr ${count} + 1`"
done
...
#${HOME}/services/cloudhost/security/firewall/UpdateNativeFirewall.sh
...
then
	${HOME}/services/dbaas/AdjustDBaaSFirewall.sh
fi
...
do
	/bin/echo "${0} `/bin/date`: Peforming online checks using curl (attempt ${count}) for newly built webserver with ip address ${ip} and private ip address ${private_ip}" 
	...
	then
		...
		then
			...
			/usr/bin/ssh -q -p ${SSH_PORT} -i ${BUILD_KEY} ${OPTIONS} ${SERVER_USER}@${private_ip}  "${SUDO} /home/${SERVER_USER}/services/webserver/RestartWebserver.sh"
			/bin/sleep 5
			count="`/usr/bin/expr ${count} + 1`"
		else
