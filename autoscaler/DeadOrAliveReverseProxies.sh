...
if ( [ ! -d ${HOME}/logs/dead_or_alive_reverse_proxy ] )
then
        /bin/mkdir -p ${HOME}/logs/dead_or_alive_reverse_proxy  
fi
...
probe_by_curl()
{
        ...
        ip="`${HOME}/services/server/GetServerPublicIPAddressByIP.sh ${ip} ${CLOUDHOST}`"
        if ( [ "${status}" = "down" ] )
        then
                ...
                ${HOME}/services/email/SendEmail.sh "IP ADDRESS REMOVED FROM DNS" "IP address of remote proxy IP address (${ip}) removed from DNS system due to an error" "ERROR"
        else
