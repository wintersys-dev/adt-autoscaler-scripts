...
build_styles_settings="`/bin/cat ${HOME}/runtime/software.dat  | /bin/grep -v "^#" | /usr/bin/gzip -f | /usr/bin/base64 | /usr/bin/tr -d '\n'`"
firewall_port_settings="`/bin/cat ${HOME}/runtime/firewall.dat | /bin/grep -v "^#" | /usr/bin/gzip -f | /usr/bin/base64 | /usr/bin/tr -d '\n'`"
application_settings="`/bin/cat ${HOME}/runtime/application.dat  | /bin/grep -v "^#" | /usr/bin/gzip -f | /usr/bin/base64 | /usr/bin/tr -d '\n'`"
...
then
	/bin/cp ${HOME}/services/server/cloud-init/${CLOUDHOST}/webserver-by-snapshot.yaml ${HOME}/runtime/cloud-init/webserver.yaml
else
...
then
	if ( [ "`/bin/grep ^NGINX:cloud-init ${HOME}/runtime/software.dat`" != "" ] )
	then
...
then
	if ( [ "`/bin/grep ^APACHE:cloud-init ${HOME}/runtime/software.dat`" != "" ] )
	then
...
then
	if ( [ "`/bin/grep ^LIGHTTPD:cloud-init ${HOME}/runtime/software.dat`" != "" ] )
	then
...
then
	if ( [ "`/bin/grep ^MARIADB:cloud-init ${HOME}/runtime/software.dat`" != "" ] )
	then
...
then
	if ( [ "`/bin/grep ^POSTGRES:cloud-init ${HOME}/runtime/software.dat`" != "" ] )
	then
...
then
	...
	PHP_VERSION="`${HOME}/utilities/config/ExtractConfigValue.sh 'PHPVERSION'`"
	php_modules="`/bin/grep ^PHP ${HOME}/runtime/software.dat | /bin/sed 's/^PHP:cloud-init://g' | /usr/bin/awk -F'|' '{print $1}' | /bin/sed 's/:/ /g'`"
	php_module_list=""
...
then
	/bin/echo "Invalid webserver cloud-init configuration found."
 	${HOME}/services/email/SendEmail.sh "INVALID CLOUD-INIT CONFIG" "Invalid webserver cloud-init configuration found." "ERROR"
fi   
