...
if ( [ "`${HOME}/utilities/config/CheckConfigValue.sh INSTALLMONITORINGGEAR:1`" = "1" ] )
then
	...
	noprovisioned="`${HOME}/services/server/GetServerIPAddresses.sh "ws-${REGION}-${BUILD_IDENTIFIER}" ${CLOUDHOST} | /usr/bin/tr '\n' ' ' | /usr/bin/wc -w`"
