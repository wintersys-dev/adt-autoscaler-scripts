...
then
        ${HOME}/services/email/SendEmail.sh "WEBSERVER TERMINATIONS HAVE BEEN PREVENTED" "WEBSERVER TERMINATIONS HAVE BEEN PREVENTED ON AUTOSCALER `/usr/bin/hostname`" "INFO"
        exit
fi
...
live_public_ips="`${HOME}/services/server/GetServerIPAddresses.sh "ws-" "${CLOUDHOST}"`"
live_ips="`${HOME}/services/server/GetServerPrivateIPAddresses.sh "ws-" "${CLOUDHOST}"`"
public_ips="`${HOME}/services/datastore/config/wrapper/ListFromDatastore.sh "config" "webserverpublicips/"`"
...
do
        ...
        then
                ${HOME}/services/datastore/config/wrapper/DeleteFromDatastore.sh "config"  "webserverpublicips/${ip}"
        fi
done
...
do
        ...
        then
                ${HOME}/services/datastore/config/wrapper/DeleteFromDatastore.sh "config"  "webserverips/${ip}"
        fi
done


NO_WEBSERVERS="`${HOME}/services/datastore/operations/ListFromDatastore.sh "scaling" "autoscaler-${autoscaler_no}/"  "scaling-${CLOUDHOST}-${REGION}" | /bin/sed -e 's/STATIC_SCALE//g' -e 's/:/ /g' -e 's/^ //g'`"
...
count="1"
candidate_termination_ips="`${HOME}/services/server/GetServerPrivateIPAddresses.sh "ws-${REGION}-${BUILD_IDENTIFIER}-${autoscaler_no}"`"
...
all_ips="`${HOME}/services/server/GetServerPrivateIPAddresses.sh "ws-${REGION}-${BUILD_IDENTIFIER}-${autoscaler_no}" ${CLOUDHOST}`"
online_ips="${all_ips}"
...
do
        if ( [ "`${HOME}/services/datastore/config/wrapper/ListFromDatastore.sh "config" "beingbuiltips/" "recursive" | /bin/grep ${ip}`" != "" ] )
        then
...
then
        ...
        do
                ...
                then
                        ...
                        ${HOME}/services/datastore/config/wrapper/ListFromDatastore.sh "config" "beenonline/${ip}"
                elif ( [ "`${HOME}/services/datastore/config/wrapper/ListFromDatastore.sh "config" "beenonline/${ip}"`" != "" ] )
                then
                        if ( [ "`${HOME}/services/datastore/config/wrapper/ListFromDatastore.sh "config" "beingbuiltips/${ip}"`" = "" ] )
                        then
                                ...
                                public_ip="`${HOME}/services/server/GetServerPublicIPAddressByIP.sh ${ip} ${CLOUDHOST}`"
...
for ip in ${online_ips}
do
        if ( [ "`${HOME}/services/datastore/config/wrapper/ListFromDatastore.sh "config" "beingbuiltips/*"  2>/dev/null | /bin/grep ${ip}`" = "" ] )
        then
                ...
                then
                        ...
                        then
                                ...
                                do
                                        /usr/bin/ssh -q -p ${SSH_PORT} -i ${BUILD_KEY} ${OPTIONS} ${SERVER_USER}@${proxy_server_ip}  "${SUDO} /home/${SERVER_USER}/services/webserver/configuration/reverseproxy/AddNewIPToReverseProxyIPList.sh ${ip}"
                                done
                        fi
